...
function parse(text) {
  ...
  matches.forEach((matchedString) => {
    ...
    if (matchedString.match(/^-=/)) {
      ...
      console.log(matchedContent, matchedOptions, " -  - - app");
    } else {
      matchedParts = matchedString.slice(1);
      matchedParts = matchedParts.match(
        /^\[([^\0]+?)\]~(?:\(([\w\d:,# ]*?)\))?$/m
      );
...
(function () {
  ...
  app.load = function () {
    ...
    require(["taskbar", "helpers", "forum/pagination"], function (
      taskbar,
      helpers,
      pagination
    ) {
  ...
  app.updateHeader = function (data, callback) {
    ...
    require([
      ...
      Promise.all(
        Object.keys(toRender).map(function (tpl) {
          return Benchpress.render("partials/" + tpl, data.header).then(
            function (render) {
  ...
  app.handleInvalidSession = function () {
    ...
    require(["translator"], function (translator) {
      translator.translate("[[error:invalid-session-text]]", function (
        translated
      ) {
        bootbox.alert({
  ...
  function highlightNavigationLink() {
    $("#main-nav li")
      ...
      .find("a")
      .filter(function (i, x) {
        return window.location.pathname.startsWith(x.getAttribute("href"));
      })
      .parent()
      .addClass("active");
  }
  ...
  app.processPage = function () {
    ...
    try {
      // 处理新老数据，展示问题，老数据保持原样，新数据采用新的结构
      let doms = $(".timeago")
      doms.each(function () {
			  const iso = this.getAttribute('title');
				if (!iso) {
					return;
				}
				let date = +new Date(iso);
				if (!isNaN(date)) {
          if (date < 1615175567583) {
            let father = $(this).parents('[component="post"]');
            father.addClass('alkun-show-toPid').removeClass('alkun-hide-toPid')
          }
				}
			}); 
    } catch (error) {
      console.log('alkun-doms-err', error);
    }
  ...
  app.showMessages = function () {
    ...
    function showAlert(type, message) {
      switch (messages[type].format) {
        ...
        case "modal":
          require(["translator"], function (translator) {
            translator.translate(message || messages[type].message, function (
              translated
            ) {
              bootbox.alert({
                title: messages[type].title,
    ...
    require(["chat"], function (chat) {
      ...
      if (chat.modalExists(roomId)) {
        ...
        socket.emit(
          ...
          { roomId: roomId, uid: uid || app.user.uid },
  ...
  app.newChat = function (touid, callback) {
    function createChat() {
      socket.emit("modules.chats.newRoom", { touid: touid }, function (
        err,
        roomId
      ) {
        if (err) {
    ...
    socket.emit("modules.chats.isDnD", touid, function (err, isDnD) {
      ...
      bootbox.confirm("[[modules:chat.confirm-chat-with-dnd-user]]", function (
        ok
      ) {
        if (ok) {
          createChat();
      ...
      require(["translator"], function (translator) {
        translator.translate(title, function (translated) {
          ...
          titleObj.interval = setInterval(function () {
            var title =
              titleObj.titles[
                titleObj.titles.indexOf(window.document.title) ^ 1
              ];
            if (title) {
  ...
  app.loadJQueryUI = function (callback) {
    ...
    scriptEl.src =
      ...
      "/assets/vendor/jquery/js/jquery-ui.js?" +
      config["cache-buster"];
  ...
  app.loadProgressiveStylesheet = function () {
    ...
    linkEl.href =
      config.relative_path + "/assets/js-enabled.css?" + app.cacheBuster;
  ...
  app.updateTags = function () {
    var metaWhitelist = ["title", "description", /og:.+/, /article:.+/].map(
      function (val) {
